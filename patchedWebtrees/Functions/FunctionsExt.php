        ...
        foreach ($indi->getChildFamilies(Auth::PRIV_HIDE) as $family) {
          ...
          foreach ($family->getSpouses(Auth::PRIV_HIDE) as $spouse) {
            if (!isset($visited[$spouse->getXref()])) {
              ...
              $node1['path'][] = $spouse;
              $node1['indi'] = $spouse;
              $node1['relations'][] = $parent_codes[$spouse->sex()];
              $p1nodes[] = $node1;
              if ($spouse === $person2) {
                $found = true;
                $resnode = $node1;
          ...
          foreach ($family->getChildren(Auth::PRIV_HIDE) as $child) {
            if (!isset($visited[$child->getXref()])) {
              ...
              $node1['path'][] = $child;
              $node1['indi'] = $child;
              $node1['relations'][] = $sibling_codes[$child->sex()];
              $p1nodes[] = $node1;
              if ($child === $person2) {
                $found = true;
                $resnode = $node1;
        ...
        foreach ($indi->getSpouseFamilies(Auth::PRIV_HIDE) as $family) {
          ...
          foreach ($family->getSpouses(Auth::PRIV_HIDE) as $spouse) {
            if (!in_array($spouse->getXref(), $node1) || !isset($visited[$spouse->getXref()])) {
              ...
              $node1['path'][] = $spouse;
              $node1['indi'] = $spouse;
              $node1['relations'][] = $spouse_codes[$spouse->sex()];
              $p1nodes[] = $node1;
              if ($spouse === $person2) {
                $found = true;
                $resnode = $node1;
          ...
          foreach ($family->getChildren(Auth::PRIV_HIDE) as $child) {
            if (!isset($visited[$child->getXref()])) {
              ...
              $node1['path'][] = $child;
              $node1['indi'] = $child;
              $node1['relations'][] = $child_codes[$child->sex()];
              $p1nodes[] = $node1;
              if ($child === $person2) {
                $found = true;
                $resnode = $node1;
      ...
      switch ($up) {
        ...
        default:
          ...
          switch ($locale->languageTag()) {
            ...
            //[RC] ADDED
            //strategy same as pl, we need different translate context though to be able to override existing translations
            //by the way, should webtrees give custom modules precedence wrt conflicting translations?
            case 'de':
              switch ($sex2) {
                case 'M':
                  if ($bef_last === 'fat') {
                    return I18N::translateContext('de: great ×(%s-1) grandfather’s brother', 'great ×%s uncle', I18N::number($up - 2));
                  } elseif ($bef_last === 'mot') {
                    return I18N::translateContext('de: great ×(%s-1) grandmother’s brother', 'great ×%s uncle', I18N::number($up - 2));
                  } else {
                    return I18N::translateContext('de: great ×(%s-1) grandparent’s brother', 'great ×%s uncle', I18N::number($up - 2));
                  }
                case 'F':
                  return I18N::translateContext('de: great ×(%s-1) grandparent’s sister', 'great ×%s aunt', I18N::number($up - 2));
                default:
                  return I18N::translateContext('de: great ×(%s-1) grandparent’s sibling', 'great ×%s aunt/uncle', I18N::number($up - 2));
              }
      ...
      switch ($down) {
        ...
        default:
          ...
          switch ($locale->languageTag()) {
            ...
            //[RC] ADDED
            case 'de': // Source: Richard Cissee
              //strategy same as pl, we need different translate context though to be able to override existing translations
              //by the way, should webtrees give custom modules precedence wrt conflicting translations?
              switch ($sex2) {
                case 'M':
                  if ($first === 'bro' && $sex1 === 'M') {
                    return I18N::translateContext('de: (a man’s) brother’s great ×(%s-1) grandson', 'great ×%s nephew', I18N::number($down - 3));
                  } elseif ($first === 'sis' && $sex1 === 'M') {
                    return I18N::translateContext('de: (a man’s) sister’s great ×(%s-1) grandson', 'great ×%s nephew', I18N::number($down - 3));
                  } else {
                    return I18N::translateContext('de: (a woman’s) great ×%s nephew', 'great ×%s nephew', I18N::number($down - 3));
                  }
                case 'F':
                  if ($first === 'bro' && $sex1 === 'M') {
                    return I18N::translateContext('de: (a man’s) brother’s great ×(%s-1) granddaughter', 'great ×%s niece', I18N::number($down - 3));
                  } elseif ($first === 'sis' && $sex1 === 'M') {
                    return I18N::translateContext('de: (a man’s) sister’s great ×(%s-1) granddaughter', 'great ×%s niece', I18N::number($down - 3));
                  } else {
                    return I18N::translateContext('de: (a woman’s) great ×%s niece', 'great ×%s niece', I18N::number($down - 3));
                  }
                default:
                  if ($first === 'bro' && $sex1 === 'M') {
                    return I18N::translateContext('de: (a man’s) brother’s great ×(%s-1) grandchild', 'great ×%s nephew/niece', I18N::number($down - 3));
                  } elseif ($first === 'sis' && $sex1 === 'M') {
                    return I18N::translateContext('de: (a man’s) sister’s great ×(%s-1) grandchild', 'great ×%s nephew/niece', I18N::number($down - 3));
                  } else {
                    return I18N::translateContext('de: (a woman’s) great ×%s nephew/niece', 'great ×%s nephew/niece', I18N::number($down - 3));
                  }
              }
